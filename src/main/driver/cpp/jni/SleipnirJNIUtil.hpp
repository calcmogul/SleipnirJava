...
jobject GetTriplets(JNIEnv* env, const Derived& mat) {
  ...
  return env->NewObject(cls, ctor, wpi::java::MakeJIntArray(env, rows),
                        wpi::java::MakeJIntArray(env, cols),
                        wpi::java::MakeJDoubleArray(env, values));
}
